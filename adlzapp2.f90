...
!  ************** testsymp.for **************************************************
  ...
  program testsymp
    ...
    !
    use coordinates,  only : mk, qdim, dof, xq, xqd, xtime, xdt
    ...
    ! Input of stepsize:
    write (*,*) "Number of steps for one cycle: "
    read  (*,*) ncall
    xdt = 1.0_mk/real(ncall,kind=mk)
    ...
    xdt = xdt*2.0_mk*pi
    write(102,*) xtime, xq(1:dof), xq(qdim+1:qdim+dof)
    ! Integration loop:
  ...
  subroutine pots1        ! dT/dp
    use coordinates, only : mk, qdim, dof, xq, xqd
    xqd(qdim+1:qdim+dof) = xq(qdim+1:qdim+dof) !xdot = dH/dp = p
    xqd(qdim+dof+1:2*qdim) = xq(qdim+dof+1:2*qdim) !vxdot = dK/dvp = vpd2H/dp2 = vp
    return
  end subroutine pots1

  subroutine pots2       ! -dV/dq
    use coordinates,  only : mk, qdim, dof, xq, xqd
    xqd(1:dof) = -xq(1:dof) !pdot = -dH/dx = -x
    xqd(dof+1:qdim) = -xq(dof+1:qdim) !vpdot = -dK/dvx = -vxd2h/dx2 = -vx
    return
